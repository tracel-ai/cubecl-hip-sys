...
The HIP libraries are loaded at runtime when the first binding is used. This means enabling the
CubeCL `hip` feature does not require ROCm to be installed on the build machine. Calls that need
HIP will fail at runtime when the libraries are unavailable; `is_available()` can be used to probe
for them first.
